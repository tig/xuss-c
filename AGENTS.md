...
4. Hardware honesty: claim readiness per spec §1. Self-report is not measurement (§6 rail 4).
5. First flash is esptool (ESP32). Subsequent app updates use the project flash path (`idf.py` / esptool). The escape hatch (`repl`/`reboot` over the ASCII link) is required from day one; a build without the door fails L1.
